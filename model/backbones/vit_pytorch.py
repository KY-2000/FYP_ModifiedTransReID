...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        num_x = img_size[1] // patch_size[1] # number of patches in width
        num_y = img_size[0] // patch_size[0] # number of patches in height
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False, SC_enable = False, sie_xishu =1.0,
                 patch_keep_rate=1.0, BDB_enable = False, batch_drop_rate=0.3, batch_drop_layer=9, PFDE_enable=False):
        super().__init__()
        ...
        self.pfde = PFDE(num_patches, self.embed_dim)
        ...
        # New Module, Patch Dropout Implementation
        self.patch_drop = PatchDropout(keep_rate = patch_keep_rate)
        print('using patch_keep_rate is : {}'.format(patch_keep_rate))

        """
        self.BDB_enable = BDB_enable
        print('BDB module: {}'.format(self.BDB_enable))
        if self.BDB_enable:
            self.batch_drop = BatchDrop(batch_drop_rate, self.patch_embed.num_x, self.patch_embed.num_y)
            self.batch_drop_layer = batch_drop_layer
            print('using batch_drop_rate is : {}'.format(batch_drop_rate))
            print('using batch_drop_layer is : {}'.format(batch_drop_layer))
        """
        # New Module, BDB Implementation
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        print("self.sie_embed: ", self.sie_embed.shape)
        print("self.cam_num: ", self.cam_num)
        print("self.sie_embed[camera_id]: ", self.sie_embed[camera_id].shape)
        print("self.sie_xishu: ", self.sie_xishu)
        print("self.sie_xishu * self.sie_embed[camera_id]:", (self.sie_xishu * self.sie_embed[camera_id]).shape)
        # print("self.sie_embed[view_id]: ", self.sie_embed[view_id].shape)
        """
        SC_feat = []
        BDB_feat = []
        ...
        # print("after embed, x.shape: ", x.shape)
        ...
        if self.local_feature:
            for i, blk in enumerate(self.blocks[:-1]):
                x = blk(x)
                """
                if((i == self.batch_drop_layer) and self.BDB_enable):
                    BDB_feat.append(self.batch_drop(x))
                """
                """
                if((i == 1 or i == 4 or i == 7) and self.SC_enable):
                    SC_feat.append(x)
                """
            return x
        else:
            ...
            return x[:, 0], BDB_feat # cls_token, skip_connection_feat.
        """
        if self.local_feature:
            for i, blk in enumerate(self.blocks[:-1]): # all blocks in the sequence but the last (for Jigsaw Branch)
                x = blk(x)
                if((i == self.batch_drop_layer) and self.BDB_enable):
                    x = batch_drop(x)
                if((i == 1 or i == 4 or i == 7) and self.SC_enable):
